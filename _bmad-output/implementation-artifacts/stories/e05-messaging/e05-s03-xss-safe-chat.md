...
# Story E5.S3: XSS-safe message rendering
...
As the platform, I want chat content sanitized so scripts cannot execute in the browser.

## Acceptance Criteria

1. Given message with HTML/script payload, when rendered, then content is escaped/sanitized.

### BDD Scenarios

- **Scenario: Acceptance 1**
  - Given message with HTML/script payload
  - When rendered
  - Then content is escaped/sanitized.
...
| FR-32 | [traceability-matrix.md](../../../planning-artifacts/traceability-matrix.md) |
...
- [prd.md](../../../planning-artifacts/prd.md) — FR-32
...
|------|-------------|--------|----------|
| 1 | Given message with HTML/script payload, when rendered, then content is escaped/sanitized. | Pass (regression) | test/Messaging.test.mo |

### BDD scenario validation

- [x] Scenario 1: message with HTML/script payload, when rendered, then content is escaped/sanitized.
