...
workflowType: story
storyId: "E7.S5"
storyKey: "e7-s05-tracking-webhooks"
epic: "E7"
phase: 2
status: done
prd: "FR-51"
...
# Story E7.S5: Tracking webhooks and timeline
...
As a buyer, I want unified tracking timeline with webhook or polling updates.

## Acceptance Criteria

1. Given TTN, when carrier updates status, then timeline reflects new state within SLA.
...
- **Scenario: Acceptance 1**
  ...
  - Then timeline reflects new state within SLA.
...
|------|-------------|--------|----------|
| 1 | Given TTN, when carrier updates status, then timeline reflects new state within SLA. | Pass (regression) | src/backend/mixins/shipping-api.mo |

### BDD scenario validation

- [x] Scenario 1: TTN, when carrier updates status, then timeline reflects new state within SLA.
