...
workflowType: story
storyId: "E2.S1"
storyKey: "e2-s01-create-listing"
epic: "E2"
phase: 1
status: done
prd: "FR-10"
...
# Story E2.S1: Create listing with photos
...
As a seller, I want to create a listing with photos, category, price, and stablecoin token so buyers can discover my offer.

## Acceptance Criteria

1. Given required fields and II auth, when I submit, then listing is active and searchable.
2. Given photo upload failure, when upload errors, then listing is not published until photos succeed.
3. Given validation errors, when I submit incomplete form, then inline errors shown.

### BDD Scenarios

- **Scenario: Acceptance 1**
  - Given required fields and II auth
  - When I submit
  - Then listing is active and searchable.
- **Scenario: Acceptance 2**
  - Given photo upload failure
  - When upload errors
  - Then listing is not published until photos succeed.
- **Scenario: Acceptance 3**
  - Given validation errors
  - When I submit incomplete form
  - Then inline errors shown.
...
- Flow: create-listing-guard
- Flow: create-listing-authenticated-form
...
|------|-------------|--------|----------|
| 1 | Given required fields and II auth, when I submit, then listing is active and searchable. | Pass (regression) | test/Marketplace.test.mo |
| 2 | Given photo upload failure, when upload errors, then listing is not published until photos succeed. | Pass (regression) | src/backend/mixins/object-storage-api.mo |
| 3 | Given validation errors, when I submit incomplete form, then inline errors shown. | Pass (regression) | src/frontend/src/pages/CreateListingPage.tsx |

### BDD scenario validation

- [x] Scenario 1: required fields and II auth, when I submit, then listing is active and searchable.
- [x] Scenario 2: photo upload failure, when upload errors, then listing is not published until photos succeed.
- [x] Scenario 3: validation errors, when I submit incomplete form, then inline errors shown.
