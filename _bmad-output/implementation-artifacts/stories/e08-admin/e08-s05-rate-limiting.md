...
# Story E8.S5: Rate limiting on public updates
...
As the platform, I want per-endpoint rate limits to reduce abuse.

## Acceptance Criteria

1. Given burst over threshold, when update called, then rate limit error returned.

### BDD Scenarios

- **Scenario: Acceptance 1**
  - Given burst over threshold
  - When update called
...
| NFR-3 | [traceability-matrix.md](../../../planning-artifacts/traceability-matrix.md) |
...
- [prd.md](../../../planning-artifacts/prd.md) — NFR-3
...
|------|-------------|--------|----------|
| 1 | Given burst over threshold, when update called, then rate limit error returned. | Pass (regression) | test/RateLimiter.test.mo |

### BDD scenario validation

- [x] Scenario 1: burst over threshold, when update called, then rate limit error returned.
