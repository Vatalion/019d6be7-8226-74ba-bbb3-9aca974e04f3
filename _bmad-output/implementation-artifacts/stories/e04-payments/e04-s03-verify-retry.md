...
# Story E4.S3: Verification retry and circuit breaker
...
As the platform, I want resilient HTTP outcalls to explorers so transient failures do not brick trades.

## Acceptance Criteria

1. Given explorer timeout, when retry policy applies, then bounded retries occur before failure.

### BDD Scenarios

- **Scenario: Acceptance 1**
  - Given explorer timeout
  - When retry policy applies
  - Then bounded retries occur before failure.
...
|------|-------------|--------|----------|
| 1 | Given explorer timeout, when retry policy applies, then bounded retries occur before failure. | Pass (known gaps) | src/backend/mixins/payments-api.mo |

### BDD scenario validation

- [x] Scenario 1: explorer timeout, when retry policy applies, then bounded retries occur before failure.
