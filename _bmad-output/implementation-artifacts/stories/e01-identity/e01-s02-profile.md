...
# Story E1.S2: Create and update profile
...
As a user, I want to set a public handle and avatar so others can recognize me without exposing private PII.

## Acceptance Criteria

1. Given first login, when I complete onboarding, then profile is persisted.
2. Given I edit profile fields, when I save, then public profile reflects changes.
3. Given another user views my profile, then sensitive fields are not exposed.
...
- **Scenario: Acceptance 1**
  ...
  - Then profile is persisted.
- **Scenario: Acceptance 2**
  - Given I edit profile fields
  - When I save
  - Then public profile reflects changes.
- **Scenario: Acceptance 3**
  - Given story preconditions are met
  - When the user completes the primary action
  - Then Given another user views my profile, then sensitive fields are not exposed.
...
|------|-------------|--------|----------|
| 1 | Given first login, when I complete onboarding, then profile is persisted. | Pass (regression) | src/frontend/src/pages/OnboardingPage.tsx |
| 2 | Given I edit profile fields, when I save, then public profile reflects changes. | Pass (regression) | src/frontend/src/pages/ProfilePage.tsx |
| 3 | Given another user views my profile, then sensitive fields are not exposed. | Pass (regression) | test/Auth.test.mo |

### BDD scenario validation

- [x] Scenario 1: first login, when I complete onboarding, then profile is persisted.
- [x] Scenario 2: I edit profile fields, when I save, then public profile reflects changes.
- [x] Scenario 3: another user views my profile, then sensitive fields are not exposed.
