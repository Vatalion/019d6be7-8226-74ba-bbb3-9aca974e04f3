...
# Story E3.S4: Refund and cancel paths
...
As a party, I want refund/cancel flows when payment or pickup fails within deadlines.

## Acceptance Criteria

1. Given eligible state, when refund requested and rules pass, then trade ends refunded.
2. Given pre-commit cancel rules, when cancel invoked, then trade terminal state is consistent.

### BDD Scenarios

- **Scenario: Acceptance 1**
  - Given eligible state
  - When refund requested and rules pass
  - Then trade ends refunded.
- **Scenario: Acceptance 2**
  - Given pre-commit cancel rules
  - When cancel invoked
  - Then trade terminal state is consistent.
...
|------|-------------|--------|----------|
| 1 | Given eligible state, when refund requested and rules pass, then trade ends refunded. | Pass (known gaps) | test/Escrow.test.mo |
| 2 | Given pre-commit cancel rules, when cancel invoked, then trade terminal state is consistent. | Pass (known gaps) | src/backend/lib/Escrow.mo |

### BDD scenario validation

- [x] Scenario 1: eligible state, when refund requested and rules pass, then trade ends refunded.
- [x] Scenario 2: pre-commit cancel rules, when cancel invoked, then trade terminal state is consistent.
